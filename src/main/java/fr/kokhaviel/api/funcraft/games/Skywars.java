...
/**
 * Funcraft Skywars Statistics
 * @author Kokhaviel
 * @since 1.0
 */
public class Skywars extends Game {

	public Skywars(Element skywarsElement) {
		final Element playerStats = skywarsElement.getElementsByClass("player-stats-game").first();
		final Element playerStatsInfo = playerStats.getElementsByClass("player-stats-info").first();

		this.setGameName(playerStats.getElementsByClass("name game-border-skywars").first().text());
